...
- 更多的选项等交互，能否为 TTFT 争取更多缓冲空间？（tip: StateMng阻塞 & 桥的位置）
- 更多的选项等交互，能否为图像生成争取更多缓冲空间？（tip: Task 非阻塞管线）
- `DECLARE` 为什么应该靠近使用点声明，而非尽量靠前？（tip: LLM 生成速率 vs 用户消费速率）
- `DECLARE` 触发生成的 `Task`，为什么不能等素材生成完毕再添加到素材库和素材列表？