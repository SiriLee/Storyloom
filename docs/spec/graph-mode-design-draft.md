...
作用：
- 记录游戏时所需要的素材，在 `saves/game_ID/_asset_list_.json` 记录
- 形式类似于一个按类型分类、以名称为键的通讯录，故称其为素材列表
...
- 素材匹配
    - 素材切换事件(`SCENE`, `SEG(char)`)触发，依据输入素材名称，强制从游戏素材列表中选择合适的素材进行匹配
    - 程序匹配：程序进行名称比较，若在素材列表中找到匹配的素材，赋值 `result` 并 `complete` ，否则由 LLM 执行
    - LLM 匹配：LLM 进行主观判定，基于列表中素材的名称和描述，选择最合适的素材赋值 `result` 并 `complete` 
- 素材生成
    - 共创阶段或素材声明事件(`DECLARE`)触发，依据声明素材的名称和描述，尝试复用库中已有素材或新生成素材
    - 程序匹配(共创阶段无)：若成功，直接 `complete` ；若失败，立即依据名称、描述构建并添加 `asset_item` ，暂不赋值 `target`
    - LLM 选择：提供游戏素材列表和素材库(部分)，由 LLM 极速判断：返回 ID 或 `NULL`(无合适素材)
...
**C.游戏素材预构建 AI**（新增）：
- 基于生成设定中的 `locations / characters` ，生成游戏的初始素材，填充素材列表（此时为空）
...
- LLM 匹配
    ...
    - LLM 接收素材的名称和描述，素材列表中名称和描述，返回选择的图像 ID

**F.素材生成 AI**（新增）：
- 若程序精准匹配名称成功（小概率），直接结束任务，无 API 请求
- **立即**依据名称、描述构建并添加 `asset_item` （暂不赋值 `target`），以防匹配任务与生成任务逻辑顺序冲突
- LLM 选择：提供游戏素材列表和素材库(按计数截取)，由 LLM 极速判断（无“思考”）：返回 ID 或 `NULL`(无合适素材)
...
- 传入角色/场景的名称、描述，以及素材库（此时游戏素材列表为空），调用 LLM 快速选择判断
- 告知 LLM 无需关注“名称”匹配度，防止名称不合适但描述契合的图像被丢弃，图像选择后在列表中重新命名
...
- 传入角色/场景的名称，以及素材列表中的名称、描述，调用 LLM 快速选择

**素材生成 Prompt**
- 传入角色/场景的名称、描述，以及素材列表和素材库（截取），调用 LLM 快速选择和判断
...
- 搭建游戏素材列表与素材库，实现：增删、计数管理、排序截取、自动清理、错误处理等基础设施
- 提供 UI 用户管理渠道
...
- 设置更多的选项等交互，是否能预留更多的 TTFT 空间？(tip: 时序模型 & 桥接机制)
- 设置更多的选项等交互，是否能预留更多的图像生成空间？(tip: Task 管线)
- `DECLARE` 在一轮中尽量靠前更好，还是使用前再声明更好？(tip: `theory/asset-generation`)
- 为什么 `DECLARE` 触发的 `Task` 不能等待素材内容生成完成后，再添加到素材库和素材列表？