...
终端 CLI、LLM 自定义变量、共创阶段变量定义、固定选项、自动存档。Web 界面（FastAPI + SSE + SPA）已提前完成。独立二进制打包（PyInstaller）+ pip wheel。版本 1.0.0。

### Phase 2 — 动态系统增强
- 变量系统增强、向量记忆、多模型
- **图像模式**：静态背景 + 角色立绘，共创预设 + 部分实时生成（规范见 `graph-mode-spec/`）

### Phase 3 — 完整体验
- 图像生成、云同步、TTS、剧本导出、多人模式
